...
// Number of recognized cache descriptors
...
@Override public String[][] getCommandText1( long[] array )
    ...
    // Return "n/a" if function returns no data
    if ( y[0] != 1 ) { return NO_RESULT; }
    // Interpreting cache descriptors
    int k=0;
    ...
    // Trim result to result1
    String[][] result1 = new String[k][NX];
    for (int i=0; i<k; i++)  // Cycle for rows
        { System.arraycopy(result[i], 0, result1[i], 0, NX); }  // Columns
    // Result is ready, all strings filled
    return result1;
    }

// Helper method, store found cache descriptor name to result text array
// INPUT:   y = array of descriptors
//          i = descriptor index
//          result = text block to build
//          k = text block index
// OUTPUT:  text block index updated
    ...
    if( z != 0 )
        ...
        for (int j=0; j<NC; j++)
            ...
            if( z == (int)CACHE_DESCRIPTORS[j][0] )
                { 
                result[k][1] = (String)CACHE_DESCRIPTORS[j][1]; 
                flag = true; 
                }
            }
