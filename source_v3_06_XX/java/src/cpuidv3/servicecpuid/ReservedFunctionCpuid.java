...
This class inherited by standard, extended, virtual and vendor-specific
...
{
    private static VENDOR_T cpuVendor = null;
    private static HYPERVISOR_T vmmVendor = null;
    static VENDOR_T getCpuVendor()                { return cpuVendor; }
    static void setCpuVendor( VENDOR_T v )        { cpuVendor = v;    }
    static HYPERVISOR_T getVmmVendor()            { return vmmVendor; }
    static void setVmmVendor( HYPERVISOR_T h )    { vmmVendor = h;    }

    EntryCpuidSubfunction[] entries;
    private EntryCpuidSubfunction[] allEntries;
    
    private int function;
    private boolean isShowFlag = false;

    final int getFunction()          { return function;   }
    final void setFunction( int f )  { function = f;      }
    final boolean isShow()           { return isShowFlag; }

    final void initData
        ( EntryCpuidSubfunction[] entries, EntryCpuidSubfunction[] allEntries )
    { 
        this.entries = entries;
        this.allEntries = allEntries;
        isShowFlag = ( entries != null ) && ( entries.length != 0 ); 
    }
    ...
    String[] getRegistersDumpUp() { return new String[] 
...
/*
    ...
    Possible optimization, see also (old) ApplicationCpuid.java,
    same functionality duplication
*/
    final static String extractVendorString
        ( EntryCpuidSubfunction entry, boolean virtual )
    {
        boolean b = false;
        int data[] = new int[3];
        data[0] = entry.ebx;
...
*/
    final static DecodeReturn decodeBitfields
        ( String regname, Object[][] regmap, int regvalue )
    {
        ...
        for (int i=0; i<n; i++)
        {
            ...
            int y1 = regvalue >> x2;  // y1 = shift parameter, required mask.
            int y2 = ~( -1 << x3 );   // y2 = mask.
            if ( y2 == 0 )
            {
                y2 = -1;
...
/*
    ...
    OUTPUT:  text array
*/
    final static ArrayList<String[]> decodeBitmap
        ( String regname, String[][] regmap, int regvalue )
    {
        ...
        for (int i=0; i<n; i++)
        {
            ...
            String[] strings = new String[]
            {
                ...
                stringSupport      // Bit comment: not supported/supported.
            };
        destination.add( strings );
        }
    return  destination;
...
*/
    ...
    final static String writeSize( int valueBits )
    {
        String s = "";                            // S = scratch string.
        int maskMega = 1 << ( valueBits - 20 );   // z2 = size in megabytes.
        int maskGiga = 1 << ( valueBits - 30 );   // z3 = size in gigabytes.
        int maskTera = 1 << ( valueBits - 40 );   // z4 = size in terabytes.
        if ( valueBits == 0 ) s = "n/a";          // width = 0 means not sup.
...
/*
    ...
    variant 1.
*/
    final static String writeAssociativityV1 ( int a )
    {
        String s = "?";
        if ( a == 0 )                   { s = "Reserved";             }
...
/*
    ...
    variant 2.
*/
    final static String writeAssociativityV2 ( int a )
    {
        String s;
        switch( a )
        {
    ...
    EntryCpuidSubfunction[] buildEntries( int f )
    {
        return ServiceCpuid.buildEntries( allEntries, f );
    }
}
