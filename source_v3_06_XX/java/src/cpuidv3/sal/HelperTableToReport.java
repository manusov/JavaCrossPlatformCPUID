...
public class HelperTableToReport 
{
    ...
    */
        ...
        ( AbstractTableModel atm, boolean strictSpaces ) 
    {
        ...
        report.append( "\r\n" );
        // Write table content.
        for (int j=0; j<n; j++)      // Cycle for rows , n = rows count.
        {
            for (int i=0; i<m; i++)  // Cycle for columns, m = columns count.
            {
                s = getShortString( atm, j, i );
    ...
    */
    private static String getShortString
        ( AbstractTableModel model, int j, int i )
    {
