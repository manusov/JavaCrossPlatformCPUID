...
public class PanelSmp extends ApplicationPanel
{
    ...
    {
        previousIndex = -1;
        treeScreenModel = CPUIDv3.getSal().getSmpTree();
    ...
    @Override AbstractTableModel[] getPanelModels() 
    {
        ...
        if ( ( treeScreenModel.downTables != null )&&
             ( treeScreenModel.downTables.length > 0 ) )
        {
            count += 1;
        }
        AbstractTableModel[] models = new AbstractTableModel[ count ];
        System.arraycopy 
            ( treeScreenModel.upTables, 0, models, 0, ( count - 1 ) );
        models[ count - 1 ] = treeScreenModel.downTables[0];
        return models;
    }
